...
### Nyquist Bit Rate  
Bit Rate = 2 * bandwidth * log<sub>2</sub>L  
(L은 신호의 레벨)
...
__신호 요소(Signal Element)__ 는 __디지털 신호(Digital Signal)__ 의 시간 상 가장 짧은 단위  

__데이터 요소(Data Element)__ 는 보내야 하는 것  

__r__ 은 각 __신호 요소__ 가 전달하는 __데이터 요소(Data Element)__ 의 개수  
...
### Biphase(Manchester)
>두 가지 전압을 사용, Bipolar(AMI, Pseudoternary)와 달리 0이 아닌 모든 비트에 대해 신호가 전송됨, 전반부(첫번째 절반)에서 전압이 한 레벨로 유지되고, 후반부(두번째 절반)에서 다른 레벨로 이동, DC component 해결,  비트 중간에서 반전되므로 Clock sync문제가 없음    
...
### Multilevel
---  
>m개 데이터 요소의 패턴을 n개 신호 요소의 패턴으로 인코딩하여 전송당 비트 수를 늘림, 한번에 더 많은 정보 전송이 가능해 대역폭 효율성 증가, mBnL 방식에서 m 데이터 요소의 패턴을 2^m<=L^n인 n 신호 요소의 패턴으로 인코딩됩니다.  
...
## 5장 아날로그 전송  
>아날로그 전송은 진폭, 주파수 또는 위상의 변화로 정보를 전송하는 방식. 이 장에서는 디지털-아날로그 및 아날로그-아날로그 변환에 사용되는 다양한 기술에 대해 논의한다.  
...
![image](https://user-images.githubusercontent.com/48265129/232298305-014825f1-bd08-4363-b863-3f013ba29536.png)  

![image](https://user-images.githubusercontent.com/48265129/232298603-cf76e2df-4b0b-4ee2-b2d4-9c639ec74015.png)  
3개로 분할한다  

![image](https://user-images.githubusercontent.com/48265129/232298880-60921592-c58a-48ad-b6f5-d3670a917274.png)  
...
![image](https://user-images.githubusercontent.com/48265129/232301470-44b544cc-c368-41df-967e-15c2f037906c.png)  
3개의 빛을 전송함  
...
![image](https://user-images.githubusercontent.com/48265129/232302432-0546f862-e00e-4d96-8a7c-32a3e2713b9c.png)  
A, B, C가 시간을 나눔  
...
![image](https://user-images.githubusercontent.com/48265129/232305603-1a5a1c18-8bfc-4fa3-8992-b3d634e6c1cc.png)  
...
### Spread spectrum  
>무선 애플리케이션에서 사용(LAN, WAN)  
>각 스테이션에는 필요보다 더 넓은 대역폭을 할당해야 함 -> 중복성 허용  
...
### Frequency Hopping Spread Spectrum(주파수 호핑 확산 스펙트럼)  
>소스 신호에 의해 변조되는 M 개의 서로 다른 반송파를 사용합니다.  
>시분할과 주파수 분할을 섞음  
...
## 7장 전송 미디어  
>전송 미디어는 소스에서 목적지까지 정보를 전달할 수 있는 모든 것으로 광범위하게 정의할 수 있고, 물리 계층 아래에 위치하며 물리 계층에 의해 직접 제어된다.   
...
### Packet Switching(패킹 전환)  
--- 
>패킷에 자원 할당 없음, 패킷 크기는 고정 or 가변적    
...
* 특징
  ...
  * 데이터 그램처럼 패킷 사용, 헤더에 주소를 포함하지만 로컬 관할권을 가짐
...
- 입력 포트, 출력 포트, 라우팅 프로세서 및 스위칭 패브릭은 패킷 스위치의 네 가지 주요 구성 요소입니다.  
- 
### 전환 방식 비교 요약
- 패킷 전환 네트워크는 전체 연결 기간 동안 자원이 전용되지 않기 때문에 회선 전환 네트워크보다 효율적이다
- 데이터 그램 네트워크는 가상 회선 네트워크보다 효율적이지만 개별 패킷 라우팅으로 인해 지연 시간이 더 김
- 가상 회선 네트워크는 회선 전환 네트워크보다 효율적이며 데이터그램 네트워크보다 지연이 적다
- 회선 전환 기술은 단순성과 신뢰성으로 인해 여전히 WAN에서 사용된다  
