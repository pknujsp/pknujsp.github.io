...
* DFA를 구성한다.
  * RE $\rightarrow$ NFA $\rightarrow$ DFA 변환 과정이 중요하다.
* 상태 최소화 기법을 사용한다.
...
## RE $\rightarrow$ DFA
---
1. RE $\rightarrow$ NFA w/$\epsilon$-moves
   - 각각의 항을 NFA로 빌드한다.
   - 그것들을 $\epsilon$-moves로 묶는다.
2. NFA w/$\epsilon$-moves 를 DFA로 만든다.
    - NFA를 시뮬레이션 하기 위해 DFA를 만든다.
    - 하위 집합을 만드는 것.
3. DFA $\rightarrow$ Minimized DFA
    - 호환 가능한 상태를 병합한다.
4. DFA $\rightarrow$ RE
    - 모든 쌍과 모든 경로들의 문제
...
이 과정을 반복한다.  
**RE $\rightarrow$ NFA w/$\epsilon$-moves $\rightarrow$ DFA $\rightarrow$ Minimized DFA $\rightarrow$ RE** ..
...
* 두 가지 핵심 함수가 있다.
  ...
  * $\epsilon$-closure(s<sub>i</sub>)
    * 상태 s<sub>i</sub>에서 $\epsilon$-전환만을 사용하여 도달 가능한 모든 상태들의 집합을 반환한다.
    * move(s<sub>i</sub>, $\epsilon$)처럼 추가 입력이 없을 경우에 바로 다음 상태로 전환이 가능할 때 사용한다.


* 알고리즘
  1. 시작 상태 설정
     * NFA의 시작 상태 s<sub>0</sub>에서 $\epsilon$-전환에 의해 도달 가능한 모든 상태들의 집합을 DFA의 시작 상태로 설정한다.
     * $\epsilon$-closure 함수를 사용해서 상태를 찾는 것이다.
  2. 새로운 상태 집합 생성
     * DFA의 상태에서 입력 기호에 대해 move 함수를 통해 가능한 상태 집합을 찾는다.
     * 찾은 상태 집합에 대해서 $\epsilon$-closure 함수를 통해, 가능한 상태 집합을 찾는다.
     * S<sub>0</sub> = $\epsilon$-closure({s<sub>0</sub>})
